...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  if(wordcount_map.count(key)) {    // BL: count actually checks whether the value exists in the map or not
    return wordcount_map.find(key)->second;   // BL: returns the 2nd number in the key/value pair - the wordcount value
  }
  else {
    return fallbackVal;
  }
