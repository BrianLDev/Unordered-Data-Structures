...
int GridGraph::countEdges() const {
  ...
  std::unordered_set<IntPairPair> uniqueEdges;
  std::unordered_set<IntPair> isolatedPoints;
  ...
  std::cout << "*** EDGE COUNT = " << numEdges << std::endl;
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  // BL NOTE: .at() references the value like a dictionary.  

  // =======================================================================
  // TODO: Your code here!
  for (auto np : originalNeighbors) {
    // take each point (np) on p1's neighbor list, look it up in the Adjacency map, return it's neighbor list, then remove p1
    std::cout << "*** Erasing : " << p1 << " from " << np << "'s neighbor list" << std::endl;
  ...
  // =======================================================================
  // TODO: Your code here!
  std::cout << "*** Erasing the point itself: " << p1 << std::endl;
...
// can be figured out based on the tile sliding rules of the puzzle.
